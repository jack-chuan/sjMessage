$(function(){
	...
	layui.use(['table','form'], function(){
	  ...
	  table.render({
	    elem: '#sjTab'
	    ,url: getRootPath()+'/sendMsg/querySendMsgList'
        ,method: 'post'
        ,where: {
	        state: 'S'
        }
	    ...
	    ,cols: [[
          ...
          ,{field:'SEND_TIME', title: '时间', align: 'center', sort: true, templet: function (d) {
                return '<span title="'+d.SEND_TIME+'">'+d.SEND_TIME+'</span>';
          }}
	  ...
	  form.render();
	  table.on('tool(sjTabF)', function (obj) {
		  if(obj.event === 'edit'){
	    	  window.location.href = getRootPath()+"/pageJump/toOutboxDetailPage?msgId="+ encodeURIComponent(obj.data.MSG_ID)+"&downloadServerIp="+ encodeURIComponent(downloadServerIp);
	      }
	  });
	  table.on('toolbar(sjTabF)', function (obj) {
	      if (obj.event === 'del') {
	    	  var data = table.checkStatus('sjTab').data;
	    	  if(data != ""){
	    		  layer.confirm('确定删除？', {
		    		  ...
		    		  $.ajax({  
		    		  	  type : "POST",
		    		  	  url : getRootPath()+'/sendMsg/deleteSendMsg',
		    		  	  dataType : "json",
		    		  	  async : false,
		    		  	  data : params,
              ...
              if(data != "" && data.length === 1){
                  if("已撤回" == data[0].STATE){
                      layer.alert('已撤回的消息不能再重复撤回！', {"skin": "layui-layer-lan","title": '系统提示'});
                  } else {
                      layer.confirm('确定撤回？', {
                          skin: "layui-layer-lan",
                          title: '系统提示'
                          ...
                          $.ajax({
                              type : "POST",
                              url : getRootPath()+'/sendMsg/backMsg',
                              dataType : "json",
                              async : false,
                              data : params,
                              success:function(iJson){
                  ...
                  layer.alert('请选择一条撤回的消息！', {"skin": "layui-layer-lan","title": '系统提示'});
              }
		  } else if(obj.event === 'for') {
              var data = table.checkStatus('sjTab').data;
              if(data != "" && data.length === 1){
                  if("已撤回" == data[0].STATE){
                      layer.alert('已撤回的消息不能再转发！', {"skin": "layui-layer-lan","title": '系统提示'});
                  } else {
                      var msgId = data[0].MSG_ID;
                      window.location.href = getRootPath()+"/pageJump/toOutboxForwardPage?msgId="+ encodeURIComponent(msgId)+"&downloadServerIp="+ encodeURIComponent(downloadServerIp);
                  }
              }else{
