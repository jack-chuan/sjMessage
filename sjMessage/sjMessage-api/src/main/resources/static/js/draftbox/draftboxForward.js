$(function(){
    ...
    $.ajax({
        ...
        success:function(iJson){
            if (iJson.code === '200'){
                ...
                // layedit.setContent(index,iJson.data.msgContent);
                $.each(iJson.data.fileList,function(i,n)
                {
                    fileIdArr.push(iJson.data.fileList[i].FILE_ID);
                    $(".uploadFileName ul").append(
                        '<li>' +
                        '<span class="uploadName">'+iJson.data.fileList[i].FILE_TITLE+'</span>' +
                        // '<input type="button" class="layui-btn layui-btn-normal layui-btn-xs dlBtn" value="下载" data-address="'+iJson.data.fileList[i].FILE_FDFS_ADDRESS+'" data-name="'+iJson.data.fileList[i].FILE_TITLE+'">' +
                        '<input type="button" class="layui-btn layui-btn-normal layui-btn-xs delBtn" value="删除" data-id="'+iJson.data.fileList[i].FILE_ID+'">' +
                        '</li>');
                });
            } else {
    ...
    layui.use(['form'],function(){
        ...
        form.on('submit(sendSubmit)', function(data){
            ...
            $.ajax({
                type : "POST",
                url : getRootPath()+'/rabbitmq/draftSendMessage',
                dataType : "json",
                async : false,
                data : params,
                success:function(iJson){
                    if ("200" == iJson.code){
                        layer.confirm("转发成功！",{
                            ...
                            btn: ['确定'], //按钮
                            cancel: function(){
                                window.location.href = getRootPath()+"/pageJump/toDraftboxPage";
                            }
                        }, function(index){
                            window.location.href = getRootPath()+"/pageJump/toDraftboxPage";
                        });
