package cn.com.sparknet.sjMessage.service;
...
import cn.com.sparknet.sjMessage.controller.ExtResultMapper;
...
public class MessageService {
	...
	public int queryRecMessageAllCount(String receiver,String mstate,String rstate,String if_read) throws Exception {
		...
		SearchRequestBuilder srb = client.prepareSearch("querymessage").setTypes("query_message");
		SearchResponse sr = srb.setQuery(QueryBuilders.boolQuery().must(QueryBuilders.matchQuery("receiver", receiver))
	...
	public int queryRecMessageByBoolQueryBuilderCount(BoolQueryBuilder boolQueryBuilder,
		...
		Client client = elasticsearchTemplate.getClient();
		SearchRequestBuilder srb = client.prepareSearch("querymessage").setTypes("query_message");
		SearchResponse sr = srb.setQuery(boolQueryBuilder.must(QueryBuilders.matchQuery("receiver", receiver))
			.filter(QueryBuilders.matchQuery("mstate", mstate))
			.filter(QueryBuilders.matchQuery("rstate", rstate))
			.filter(QueryBuilders.matchQuery("if_read", if_read))).setSize(10000).execute().actionGet();
		SearchHits hits = sr.getHits();
	...
	private NativeSearchQuery getNativeSearchQueryWithBoolQuery(int pageNum, int pageSize, BoolQueryBuilder boolQueryBuilder,
		...
		NativeSearchQuery query = nativeSearchQueryBuilder.withQuery(boolQueryBuilder)
				...
				.withFilter(QueryBuilders.boolQuery()
					.must(QueryBuilders.matchQuery("receiver", receiver))
					.filter(QueryBuilders.matchQuery("mstate", mstate))
					.filter(QueryBuilders.matchQuery("rstate", rstate))
					.filter(QueryBuilders.matchQuery("if_read", if_read)))
				.withHighlightFields(
	...
	private NativeSearchQuery getNativeSearchQuery(int pageNum, int pageSize,NativeSearchQueryBuilder nativeSearchQueryBuilder,
		...
		NativeSearchQuery query = nativeSearchQueryBuilder.withPageable(new PageRequest(pageNum, pageSize))
			.withSort(SortBuilders.fieldSort("create_date").order(SortOrder.DESC))
			.withFilter(QueryBuilders.boolQuery()
				.must(QueryBuilders.matchQuery("receiver", receiver))
				.filter(QueryBuilders.matchQuery("mstate", mstate))
	...
	public BoolQueryBuilder getBoolQueryBuilder(String msg_title,String msg_type,String msg_content,String deptname,
		...
		if(msg_title != null && !"".equals(msg_title)) {
			boolQueryBuilder.must(QueryBuilders.termQuery("msg_title", msg_title));
		}
		if(msg_type != null && !"".equals(msg_type)) {
			boolQueryBuilder.must(QueryBuilders.termQuery("msg_type", msg_type));
		}
		if(msg_content != null && !"".equals(msg_content)) {
			boolQueryBuilder.must(QueryBuilders.termQuery("msg_content", msg_content));
		}
		if(deptname != null && !"".equals(deptname)) {
			boolQueryBuilder.must(QueryBuilders.termQuery("deptname", deptname));
		}
		if(orgname != null && !"".equals(orgname)) {
			boolQueryBuilder.must(QueryBuilders.termQuery("orgname", orgname));
		}
		if(sender != null && !"".equals(sender)) {
			boolQueryBuilder.must(QueryBuilders.termQuery("sender", sender));
		}
